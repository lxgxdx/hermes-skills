...
Use the linked template at `references/templates/pmb-codex-lane-prompt.md` (originally `templates/pmb-codex-lane-prompt.md` in the standalone `kanban-codex-lane` skill) for prediction-market-bot work. For other repos, keep the same structure and replace the PMB-specific safety block with repo-specific invariants.
